...
class ProductService:
    ...
    def create_product(self, product_data: ProductData) -> ProductData:
        return self.storage.save(product_data)

    def get_products(self) -> list[ProductData]:
        return self.storage.get()

    def get_product_by_id(self, product_id: int) -> ProductData | None:
        return self.storage.get_by_id(product_id)

    def get_products_by_user_id(self, user_id: int) -> list[ProductData]:
        all_products = self.get_products()
        return [product for product in all_products if product.user_id == user_id]

    def delete_product(self, product_id: int) -> ProductData | None:
